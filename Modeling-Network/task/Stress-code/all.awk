...
#               pkt_end_time[packetid] = simulation_end; # EXPERIMENTAL
    ...
    for (i in nodes) {
        if ( packets_received[i] > 0 ) {
            ...
            if ( runtime > 0 ) {
                ...
                printf("%d %f %f %d\n", i, start, end, throughput[i]) >> "tcl/ex/Sid/Risultati/throughput.dat";
            }
        }
        ...
        if ( packets_sent[i] > 2 ) {
            ...
            if ( runtime > 0 ) {
                ...
                printf("%d %f %f %d\n", i, start, end, rate[i]) >> "tcl/ex/Sid/Risultati/rate.dat";
            }
        }
    }
    ...
    for ( pkt in pkt_end_time) {
        ...
        if ( delta > 0 ) {
            delay[pkt] = delta;
            printf("%d %f %f %f\n", pkt, start, end, delta) >> "tcl/ex/Sid/Risultati/delay.dat";
        }
    }
    ...
    if ( total_runtime > 0 && total_packets_sent > 0)
        load = ((total_packets_sent * 8000)/total_runtime) / 2000000; # n=o overhead
